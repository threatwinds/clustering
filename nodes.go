...
func (node *node) connect() {
	...
	go_sdk.Logger().LogF(200, "connecting to %s", node.properties.NodeIp)

	err := go_sdk.Logger().Retry(func() error {
		conn, err := grpc.Dial(
	...
	if err != nil {
		e := go_sdk.Logger().ErrorF("error connecting to %s: %v", node.properties.NodeIp, err)
...
func (node *node) startSending() {
	...
	if err != nil {
		e := go_sdk.Logger().ErrorF("error sending task: %v", err)
		node.setUnhealthy(e.Message)
		return
	}
	for {
...
func (node *node) joinTo(newNode *node) {
	...
	if err != nil {
		e := go_sdk.Logger().ErrorF("error registering from %s to %s, %v", node.properties.NodeIp, newNode.properties.NodeIp, err)
		node.setUnhealthy(e.Message)
	}
}
...
func (node *node) updateTo(dstNode *node) {
	...
	if err != nil {
		e := go_sdk.Logger().ErrorF("cannot send resources update to %s: %v", dstNode.properties.NodeIp, err)
...
func (node *node) withLock(ref string, action func() error) *logger.Error {
	...
	case <-time.After(10 * time.Second):
		return go_sdk.Logger().ErrorF("%s: timeout waiting to lock %s", ref, node.properties.NodeIp)
	case node.mutex <- struct{}{}:
		defer func() {
			<-node.mutex
			go_sdk.Logger().LogF(100, "%s: released node %s", ref, node.properties.NodeIp)
		}()
		go_sdk.Logger().LogF(100, "%s: locked node %s", ref, node.properties.NodeIp)
	}
